const stylesProfile = {
    container: 'flex-1 bg-secondary relative', //Profile
    container1: 'flex-1 bg-primary relative', //ProfileUpdate

    //// Background 
    containerBg1: 'absolute bottom-0 w-full h-4/6 bg-principal rounded-t-3xl pt-12 px-6',
   
    //// Image
    containerImage:"w-full justify-center absolute items-center h-1/6 pb-20 ml-6",
    image:"w-32 h-32 rounded-full",
    iconImage: "w-32 h-32 rounded-full border-2 border-primary justify-center items-center",
    image1:"w-32 h-32 rounded-full mx-auto",
  
    //// Data
    containerRow: "flex-row items-center justify-center w-full space-x-1 pt-1",
    titleFlex: "mx-1 w-1/5 font-montserrat text-center text-l my-3 text-primary", 

    container3: "items-center justify-center pt-4 mb-5 h-12", //Profile
    container4: "justify-center items-center mx-2", //Config

    title: "text-primary font-montserrat text-center pt-12 text-3xl px-22 m-4" , //Update
    title1: "text-base font-montserrat text-center pt-12 text-xl px-22", // Profile
    title3: " font-montserrat text-left my-3 text-primary text-l",   //Config
    
    inputContainer: "flex-row items-center bg-bgInput w-full mb-4 p-3 h-12",//Update
    input: 'mx-1 w-4/5 h-full',
  
  ///Button
    button: ' flex-row w-full h-12 items-center justify-center bg-terciary rounded-l ', //Profile
    buttonText: ' ml-2 h-7 text-primary text-lg font-montserrat',
    button1: ' flex-row w-full h-12 items-center justify-center bg-primary rounded-xl mt-8', //Config
    buttonText1: ' ml-2 h-7 text-principal text-lg font-montserrat',

    //Button back
    buttonBack1: 'w-full h-12 items-center justify-center bg-primary rounded-lg', //Profile
    buttonBack2: 'w-full h-12 items-center justify-center bg-secondary rounded-lg', //UpdateProfile
    containerBack: "flex justify-start w-3/12 p-7 mt-2",
    };
    
    export default stylesProfile;